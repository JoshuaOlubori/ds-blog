# Conversation
- conversationId: 6a58c122-2ff1-4552-a5e3-6fdbe5ba50a9
- endpoint: Anthropic
- title: Guía Completa Procedimientos Almacenados
- exportAt: 14:40:54 GMT+0100 (West Africa Standard Time)
...
- messages: 696e2bffa92ad5e523019011,696e2c41a92ad5e52301940a,696e2c4da92ad5e5230194b4,696e2c90a92ad5e5230198ae,696e2c9ba92ad5e52301996a,696e2ce2a92ad5e523019dc2,696e2cf0a92ad5e523019ec1,696e2d2ea92ad5e52301a2e1,696e2d39a92ad5e52301a38b,696e2d7da92ad5e52301a779,696e2d95a92ad5e52301a8fc,696e2ddca92ad5e52301ad87,696e2e0aa92ad5e52301b075,696e2e4ea92ad5e52301b4ec,696e2e62a92ad5e52301b618,696e2eaca92ad5e52301ba8b,696e2f12a92ad5e52301c1ba,696e2f63a92ad5e52301c776,696e2f6fa92ad5e52301c84d,696e2fb8a92ad5e52301cc9d,696e2fd7a92ad5e52301ce71,696e3014a92ad5e52301d327
- tools: 
- model: vertex_ai.anthropic.claude-opus-4-5
- tags: 
- createdAt: 2026-01-19T13:05:03.147Z
- updatedAt: 2026-01-19T13:22:28.102Z
- resendFiles: true
- _id: 696e2bffa92ad5e523019012
- __v: 0
- files: 

## History
**User**
**Role:** You are a principal data architect with 18+ years of experience designing data platforms that support both operational applications and analytical workloads. Your expertise spans building maintainable stored procedure libraries for mission-critical systems, architecting view layers for complex enterprise data models, and optimizing 
SQL for analytics at terabyte-scale. You have led teams through migrations from procedural database logic to modern patterns, and have extensive experience tuning analytical queries for data warehouses and OLAP systems.

**Task:** Write a comprehensive technical guide covering the following core topics:

1. **Stored Procedures** – Creation syntax, parameter handling (input, output, default values), control flow structures, error handling and transaction management within procedures, security considerations (ownership chaining, EXECUTE AS), performance implications, and when to use vs. avoid procedural logic in the database

2. **Views vs. Materialized Views** – Standard view mechanics (query substitution, updateability, security layer benefits), materialized/indexed views (persistence, refresh strategies, storage trade-offs), query optimizer interactions, dependency management, and architectural patterns for when each is appropriate

3. **Analytics-Heavy SQL** – Window functions (ranking, aggregate, offset functions), advanced GROUP BY extensions (ROLLUP, CUBE, GROUPING SETS), CTEs for recursive and analytical patterns, pivot/unpivot operations, running totals and moving averages, percentile calculations, and query optimization strategies for large-scale analytical workloads

**Audience:** Junior-to-mid-level data engineers and analytics engineers who are comfortable with intermediate SQL (JOINs, subqueries, basic aggregations) and are now moving into roles requiring them to write production-grade procedural code, design abstraction layers over complex schemas, and build performant analytical queries. Assume no prior experience with stored procedure development, limited exposure to materialized views, and basic familiarity with window function syntax but not advanced analytical patterns.

**RDBMS Focus:**

- **Primary:** SQL Server and PostgreSQL (provide complete syntax, system catalog views, tooling, and behavioral specifics for each where relevant)
- **Secondary:** Note significant differences in MySQL (stored procedure limitations, lack of native materialized views), Oracle (PL/SQL distinctions, advanced materialized view refresh options), and modern analytical engines (Snowflake, BigQuery, Redshift) where applicable for analytical SQL patterns

**Required Elements:**

### Structure
Use clear hierarchical headings (H1/H2/H3), begin with an executive summary, and follow a logical flow from foundational concepts (basic stored procedures, simple views) through intermediate topics (parameterized procedures, materialized view refresh strategies, window function families) to advanced techniques (complex analytical patterns, procedural error handling, query optimization for analytics).

### Diagrams
Include Mermaid diagrams to illustrate:

- **Stored procedure execution flow** (parameter binding → compilation/caching → execution → result return → error handling paths)
- **View resolution process** (showing how the optimizer merges view definitions into the outer query)
- **Materialized view lifecycle** (creation → initial population → staleness → refresh strategies → invalidation)
- **Window function frame visualization** (showing ROWS vs. RANGE, PRECEDING/FOLLOWING boundaries on sample data)
- **ROLLUP/CUBE/GROUPING SETS hierarchy** (visual breakdown of which grouping combinations each produces)
- **Analytical query execution plan patterns** (showing sort, hash aggregate, window aggregate operators and data flow)

### Case Studies
Provide 4–6 realistic, illustrative scenarios showing:

- **The initial problem** (symptoms such as code duplication across applications, stale dashboard data, slow analytical reports, unmaintainable view chains, timeout errors on aggregation queries, or security/audit gaps)
- **Root cause analysis** (what design decision, missing optimization, or architectural gap caused the issue)
- **The solution implemented** (procedure refactoring, materialized view introduction, query rewrite using window functions, index strategy for analytical patterns, etc.)
- **Measurable outcomes** (reduced code maintenance burden, query time improvements, freshness guarantees met, reduced compute costs)

**Example scenarios to address:**
- A reporting system where identical complex logic is duplicated across 12 application queries, leading to inconsistencies
- A dashboard with a 47-second load time due to computing aggregations on-the-fly against billions of rows
- A stored procedure that worked in development but causes blocking and timeouts under production concurrency
- A nested view architecture (5+ levels deep) that the optimizer cannot flatten, causing severe performance issues
- A financial reporting query requiring running totals, period-over-period comparisons, and percentile rankings that initially took 8 minutes

### Mistakes & Fixes
Explicitly call out common mistakes, including:

**Stored Procedures:**
- Excessive procedural logic that should be set-based operations
- Missing or improper error handling (losing errors, incomplete rollbacks)
- Parameter sniffing issues causing plan cache problems
- Security vulnerabilities from dynamic SQL without proper parameterization
- Procedures that hold locks excessively due to embedded transactions

**Views:**
- Creating deeply nested view hierarchies that confuse the optimizer
- Assuming views provide performance benefits (they don't inherently)
- Using SELECT * in view definitions (schema binding issues)
- Not understanding when views are updatable vs. read-only
- Ignoring materialized view maintenance costs and staleness

**Analytics SQL:**
- Using correlated subqueries instead of window functions (N+1 performance pattern)
- Incorrect window frame specifications leading to wrong results
- Over-using DISTINCT or GROUP BY when window functions would be cleaner
- Not leveraging GROUPING SETS when multiple aggregation levels are needed
- Ignoring sort operations required by window functions in execution plans

For each mistake: **how to detect it** (execution plans, profiling, catalog queries, code review patterns) and **how to prevent or correct it**

### Code Examples
Include real SQL snippets demonstrating:

**Stored Procedures:**
- Complete procedure creation with input/output parameters in SQL Server and PostgreSQL
- Control flow structures (IF/ELSE, WHILE/LOOP, CASE)
- TRY-CATCH error handling (SQL Server) and EXCEPTION blocks (PostgreSQL)
- Transaction management within procedures (explicit commits, savepoints, nested transaction behavior)
- Dynamic SQL construction with proper parameterization
- Table-valued parameters (SQL Server) and array/JSON parameter patterns (PostgreSQL)

**Views:**
- Standard view creation with and without SCHEMABINDING/schema dependencies
- Updatable views with CHECK OPTION
- Indexed/materialized view creation, including required conditions (SQL Server indexed views, PostgreSQL materialized views)
- Refresh strategies (full refresh, incremental where supported, concurrent refresh in PostgreSQL)
- System catalog queries to inspect view definitions and dependencies

**Analytical SQL:**
- All major window function categories with clear examples:
  - Ranking: ROW_NUMBER, RANK, DENSE_RANK, NTILE
  - Aggregate: SUM, AVG, COUNT, MIN, MAX as window functions
  - Offset: LAG, LEAD, FIRST_VALUE, LAST_VALUE, NTH_VALUE
- Window frame specifications (ROWS BETWEEN, RANGE BETWEEN, GROUPS BETWEEN in PostgreSQL 11+)
- ROLLUP, CUBE, GROUPING SETS with GROUPING() function for null disambiguation
- Recursive CTEs for hierarchical data (org charts, bill of materials, graph traversal)
- PIVOT/UNPIVOT (SQL Server native syntax, PostgreSQL crosstab alternatives)
- Running totals, moving averages, cumulative distribution, percent of total patterns
- Percentile functions (PERCENTILE_CONT, PERCENTILE_DISC, PERCENT_RANK, CUME_DIST)

**Good vs. Bad Practice Annotations:**
- Annotated examples showing the same problem solved poorly (cursor-based, correlated subqueries) vs. well (set-based, window functions)
- Before/after refactoring examples with performance metrics

### Practical Guidance
Include decision frameworks or checklists for:

- **When to use stored procedures vs. application code** (considering maintainability, security, performance, portability, and team skills)
- **Choosing between views, materialized views, and tables** (based on query frequency, data volatility, freshness requirements, and storage constraints)
- **Selecting the right analytical SQL approach** (flowchart: subquery vs. CTE vs. window function vs. GROUP BY extension)
- **Optimizing analytical queries** (checklist covering indexes for window functions, pre-aggregation strategies, partition pruning, and parallel execution)
- **Materialized view refresh strategy decision matrix** (real-time requirements, data volume, acceptable staleness, compute budget)

### Comparison Tables
Include reference tables for:

- Window function syntax differences across SQL Server, PostgreSQL, MySQL, and Oracle
- Materialized view capabilities by platform (refresh options, query rewrite, incremental refresh support)
- Stored procedure feature support matrix (output parameters, table-valued returns, autonomous transactions, etc.)
- GROUP BY extension equivalencies (showing how ROLLUP/CUBE translate to UNION ALL of GROUP BY combinations)

**Tone:** Technically rigorous but accessible. Use clear explanations for complex concepts, especially when demonstrating why one approach is superior to another. Avoid unnecessary jargon without sacrificing precision. Use analogies where helpful (e.g., comparing materialized views to a cached report printed and pinned to a bulletin board vs. running to the filing cabinet for every request; comparing window functions to a spreadsheet formula that can reference other rows in a sorted range).

**Length:** Aim for a thorough, reference-quality document (approximately 5,000–7,000 words or equivalent depth, as analytical SQL patterns require extensive examples).


**Anthropic**
# Comprehensive Technical Guide: Stored Procedures, Views, and Analytics-Heavy SQL
