...
<iframe
  width="560"
  height="315"
  src="https://drive.google.com/file/d/1vYEH2rd6cEtmyBuogWvNGgl65WisQZdb/preview" 
  title="video player"
...
![WHERE clause](/static/images/syntax-illustrations/where-clause-1.png)
...
A useful query is to filter on missing data. Perhaps you need to know where missing data is present or absent in your table,
you could do this:
...
SELECT *
FROM employee_demographics
WHERE birth_date IS NULL; -- this would return employees that have not registered their birthdates
```
<Note>
In databases, if data is not inserted into a record, that state of absense is represented as `NULL`.
</Note>

If you want to return employees who actually have birth dates, you would use the `NOT` modifier like so:
...
<Note>
This is a good time to introduce comments in SQL.
Anything after two consecutive hyphens `(--)` is ignored by the SQL engine and therefore can serve as documentation for code.
Always document your code, especially sections whose function is not immediately obvious.

Multi-line (block) comments are enclosed within `/*` and `*/`. All content between these delimiters, 
including text spanning multiple lines, is treated as a comment.
For example:
...
![WHERE clause](/static/images/syntax-illustrations/like-clause-1.webp)
...
3. **How many unique job titles exist in the organization?**
   ...
   <details>
     <summary>Show Solution</summary>
     ```sql 
     SELECT COUNT(DISTINCT occupation) AS unique_job_titles
      FROM employee_salary; 
      ```
   </details>
...
<iframe
  width="560"
  height="315"
  src="https://drive.google.com/file/d/1CXvk59O_3EdfhWp6uT8m2-40iwea5rPE/preview"
  title="video player"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
>
